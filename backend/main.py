...
def _apply_user_update(payload: schemas.UserUpdateIn,
                       db: Session,
...
def update_me_patch(payload: schemas.UserUpdateIn,
                    db: Session = Depends(get_db),
                    user: models.User = Depends(get_current_user)):
    return _apply_user_update(payload, db, user)


@app.put("/auth/me", response_model=schemas.UserOut)
def update_me_put(payload: schemas.UserUpdateIn,
                  db: Session = Depends(get_db),
                  user: models.User = Depends(get_current_user)):
    return _apply_user_update(payload, db, user)


@app.post("/auth/me/update", response_model=schemas.UserOut)
def update_me_post(payload: schemas.UserUpdateIn,
                   db: Session = Depends(get_db),
                   user: models.User = Depends(get_current_user)):
    return _apply_user_update(payload, db, user)
